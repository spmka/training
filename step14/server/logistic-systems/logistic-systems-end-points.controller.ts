...
import {LogEntriesProvider} from '../log-entries/log-entries.data';
...
export class LogisticSystemsEndPointsController extends AbstractEndPointsController {
  ...
  private handleGetLogEnties(response: Response, systemId: number) {
    const fillteredlogEntries = new LogEntriesProvider().getLogEntries()
      .filter(logEntry => logEntry.systemId === systemId);
