...
import {HardwareUsagesProvider} from './hardware-usages.data';

/** Defines all routes belonging to log entries */
export class HardwareUsagesEndPointsController extends AbstractEndPointsController {
  ...
  private handleGetAll(response: Response, systemId: number) {
    const hardwareUsages = new HardwareUsagesProvider().getHardwareUsages();
    if (systemId) {
