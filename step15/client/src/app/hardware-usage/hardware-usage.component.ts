...
import {HardwareUsageList} from '../../../../server/hardware-usage/hardware-usage.model';
...
export class HardwareUsageComponent implements OnInit {
  ...
  public threadUsageList: AbstractHardwareUsageList;
  ...
  private async loadHardwareUsage() {
    ...
    if (result.isOk()) {
      ...
      this.threadUsageList = this.hardwareusages.map(hwUsage => ({timestamp: hwUsage.timestamp, value: hwUsage.threads}));
