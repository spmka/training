...
import {logEntriesFilterFormIds} from './log-entries-filter.form-config';
...
export class LogEntriesFilterComponent implements OnInit {
  /** formcontrol ids used in the class and in the html template */
  public formIds = logEntriesFilterFormIds.filterDialog;
  ...
  private createForm() {
    this.filterFormGroup = new FormGroup({
      [this.formIds.logisticSystem]: new FormControl('', [Validators.required]),
      [this.formIds.logFilter]: new FormControl('', [Validators.required])
    });
    this.filterFormGroup.get(this.formIds.logisticSystem)
    ...
    this.filterFormGroup.get(this.formIds.logFilter)
